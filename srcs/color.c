...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/24 17:14:03 by ahirzall          #+#    #+#             */
/*   Updated: 2025/03/24 17:22:40 by ahirzall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		unsigned int color2)
{
	...
	r = (unsigned int)(((1.0 - t) * ((color1 >> 16) & 0xFF)) + (t
				* ((color2 >> 16) & 0xFF)));
	g = (unsigned int)(((1.0 - t) * ((color1 >> 8) & 0xFF)) + (t
				* ((color2 >> 8) & 0xFF)));
	b = (unsigned int)(((1.0 - t) * (color1 & 0xFF)) + (t * (color2 & 0xFF)));
	result = (r << 16) | (g << 8) | b;
	return (result);
}
 ...
 * Generates a color using the given palette.
	...
	* The iteration count is normalized (with fixed sharpness = 256)
	and mapped to a palette index.
 */
 ...
 * put_pixel
 *
 * Draws a pixel at coordinates (x,
	y) on the given image with the specified color.
 * The function assumes that the image's data address, line length,
	and bits per pixel
 * have been set up by a prior call to mlx_get_data_addr.
 * - If the point does not escape within the maximum number of iterations (i.e.,
	i reaches iterations_depth),

	*      it is considered inside the fractal and is colored black
		(or the default color):
 * - Once a point is determined to have escaped (i.e.,
	the magnitude exceeds the escape threshold),

	*      the corresponding pixel is colored. The number of iterations
		(i) it took for z to escape is used to generate the color.
...
void	put_pixel(int x, int y, t_img *img, int color)
{
	...
	dst = img->img_pixels_ptr + y * img->size_line + x * (img->bits_per_pixel
			/ 8);
